...
function baseProg(){return{done:{},answerStats:{},history:[],mistakes:{},themeStats:{},catStats:{},daily:{},dailyStems:{},recentIds:[],recentThemes:[],settings:{todayCount:21,recentBlock:30,masterNeed:2,correctCooldown:14,masterCooldown:30,easyCooldown:60,maxKnownToday:2}}}
...
function categoryProgress(c,i,p){const total=qsCat(c.id).length,done=solvedCountByCat(p,c.id),rate=pct(done,total);return `<div class="cat-progress" aria-label="${c.name} 達成率 ${rate}%"><div class="cat-progress-line"><span>${i+1} ${c.name}</span><em>${rate}%　${done} / ${total} 問 完了</em></div><div class="cat-progress-track"><i style="width:${rate}%"></i></div></div>`}
...
function question(){let q=currentQ(),b=base(q),c=cat(b.gid),s=sub(q.subject),ci=state.view.choices.findIndex(x=>x.correct),ok=state.answered&&state.view.choices[state.selected]?.correct;return `<main>${header(`${state.mockEnd?'模試':'第'+(state.idx+1)+'問'} / ${state.session.length}問`,true)}${state.answered?`<div class="resultTop ${ok?'ok':'ng'}">${ok?'⭕':'❌'}<span>${ok?'正解':'不正解'}</span>${!ok?`<span>正解 ${jpNo[ci]} ${state.view.choices[ci].text}</span>`:''}</div>`:''}<section class="content questionContent ${state.answered?'withResult':''}"><div class="qcard question-card ${importanceClass(q)}"><div class="qmeta"><span>${s.name} ＞ ${c.name}</span><span>${importanceBadge(q)}・4択</span></div><p class="qtext">${b.question}</p>${state.view.choices.map((ch,i)=>choiceBtn(ch,i)).join('')}</div>${state.answered?answerBox(q,ci,b):''}</section><div class="fixedNext"><button onclick="${state.answered?'nextQ()':'answer()'}">${state.answered?'次へ':'解答する'}</button></div>${bottom('today')}</main>`}
...
function score(){const p=prog();if(state.mockEnd){const recent=(p.history||[]).slice(-state.session.length),ok=recent.filter(x=>x.ok).length,pass=ok>=33;return `<main>${header('模試結果',true)}<section class="content"><div class="qcard"><h2>${pass?'合格！':'不合格！'}</h2><p class="score">${ok} / ${state.session.length}</p></div></section>${bottom('score')}</main>`}return `<main>${header('学習記録',true)}<section class="content"><div class="qcard"><h2>正解済み</h2><p class="score">${solvedIds(p).size} / ${QUESTIONS.length}</p></div></section>${bottom('score')}</main>`}
