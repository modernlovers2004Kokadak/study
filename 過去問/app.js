...
const jpNo=['1','2','3','4'];
let state={page:'home',session:[],idx:0,selected:null,answered:false,view:null,filter:'all',themeFilter:null,search:'',mockEnd:null,timer:null,mockRecorded:false,mockResults:[]};
...
function startSession(arr,page='question',mock=false){if(!arr||!arr.length){alert('出題できる問題がありません。');route('home');return}state={...state,page,session:arr,idx:0,selected:null,answered:false,view:null,mockEnd:mock?Date.now()+60*60*1000:null,mockRecorded:false,mockResults:[]};if(mock)saveMockProgress();render();}
...
function submitAnswer(){if(state.selected===null)return;const q=currentQ(),ok=!!state.view.choices[state.selected]?.correct;record(q,ok);if(state.mockEnd)state.mockResults.push({id:q.id,ok});state.answered=true;if(state.mockEnd)saveMockProgress();render()}
